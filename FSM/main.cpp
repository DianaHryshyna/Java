...
Vector2D *hello = new Vector2D(50, 66);
...
void Ant::findLeaf() {
    ...
    if(distance(hello, this->position) <= 30) this->brain->pushState(bind(&Ant::sayHello,this));
...
void Ant:: sayHello() {
    cout << "Przewitaj sie! Bro jest obok" << endl;
    this->velocity = new Vector2D(position->x - hello->x, position->y - hello->y);
    if(distance(hello, this->position) > 30) {
        this->brain->popState();
    }
}

int main() {
    Ant *obj = new Ant(50, 80);
    while(true) {
